...
function token_verify(is_refresh, access_token, refresh_token, username_stored, redirect = true) {
    ...
    req.onload = function () {
        if (req.status >= 200 && req.status < 300) {
            ...
            console.log(data)
            // example result: {"code":"400","status":"Failed","message":"Signature verification failed. Token has expired. Login agian","result":null}
            ...
            if (data.code == "200" || data.code == "201") {
                ...
                // Check if the user already has a history uploaded
...
// This method can detect if the current user already has a history uploaded to the backend or if this needs to be done first (no history yet)
